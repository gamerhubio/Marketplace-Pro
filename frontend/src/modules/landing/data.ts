export const gamerhubSlider = [
    {
        img: "/images/landing/gamerhub-slider/slider1.png",
        width: 765,
    },
    {
        img: "/images/landing/gamerhub-slider/slider1.png",
        width: 765,
    },
    {
        img: "/images/landing/gamerhub-slider/slider1.png",
        width: 765,
    },
    {
        img: "/images/landing/gamerhub-slider/slider1.png",
        width: 765,
    },
];

export const streamingSlider = [
    {
        img: "/images/landing/streaming-slider/streaming-img.png",
        video: "",
        link: "https://google.com",
    },
    {
        img: "/images/landing/streaming-slider/streaming-img.png",
        video: "",
        link: "https://google.com",
    },
    {
        img: "/images/landing/streaming-slider/streaming-img.png",
        video: "",
        link: "https://google.com",
    },
    {
        img: "/images/landing/streaming-slider/streaming-img.png",
        video: "",
        link: "https://google.com",
    },
];

export const marketplaceSlider = [
    "/images/landing/marketplace-slider/marketplace-img.png",
    "/images/landing/marketplace-slider/marketplace-img.png",
    "/images/landing/marketplace-slider/marketplace-img.png",
    "/images/landing/marketplace-slider/marketplace-img.png",
];

export const merchSlider = [
    "/images/landing/merch-slider/merch-img.png",
    "/images/landing/merch-slider/merch-img.png",
    "/images/landing/merch-slider/merch-img.png",
    "/images/landing/merch-slider/merch-img.png",
];

export const tokenomicsData = [
    {
        title: "Seed Sale Price",
        value: "1 GHT = $0.005",
    },
    {
        title: "Private Sale Price",
        value: "1 GHT = $0.01",
    },
    {
        title: "Public Sale Price",
        value: "1 GHT = $0.015",
    },
    {
        title: "Total Supply",
        value: "1, 000, 000, 000",
    },
];

export const tokenomicsCardData = [
    {
        img: "/images/landing/tokenomics/token1.png",
        title: "Ecosystem Currency",
        description:
            "GHT will be the major gaming and trading currency on GameHub as well as the NFT & In-game asset marketplaces.",
    },
    {
        img: "/images/landing/tokenomics/token2.png",
        title: "GameFi and MetaFi",
        description:
            "GHT will be used for game token mining and other GameFi services.",
    },
    {
        img: "/images/landing/tokenomics/token3.png",
        title: "Ecosystem Governance",
        description:
            "GHT serves as the governance token for the GamerHub ecosystem.",
    },
    {
        img: "/images/landing/tokenomics/token4.png",
        title: "Exclusive Access",
        description:
            "GHT holders are granted access to exclusive games and streaming content, events and demos",
    },
    {
        img: "/images/landing/tokenomics/token5.png",
        title: "Merchandise",
        description:
            "GHT is the main currency for branding and trade of merchandise",
    },
];

export const roadmapData = [
    {
        desc: ["Demo & MVP Development", "Game & User Profile Integration"],
        year: "2023",
        month: "Q1 - Q4",
    },
    {
        desc: [
            "PC Launcher Release (V1 & V2)",
            "In-game Asset Marketplace Launch",
        ],
        year: "2024",
        month: "Q1 - Q4",
    },
    {
        desc: [
            "Gamer Reward System Expansion",
            "DAO Framework Integration",
            "Game Hackathons",
        ],
        year: "2025",
        month: "Q1 - Q3",
    },
    {
        desc: [
            "Web2 to Web3 SDKs",
            "Metaverse & Inter-metaverse Competitions",
            "Cross-platform Interoperability",
        ],
        year: "2025",
        month: "Q4",
    },
    {
        desc: ["AI Game Companion", "GamerHub AI/VR Metaverse Development"],
        year: "2026",
        month: "Q1 - Q4",
    },
    {
        desc: [
            "Tokenization of AI Game Assets & Elements",
            "SDK & API Development",
            "ZK Tech & zkNFT Marketplace/Bridge",
        ],
        year: "2027",
        month: "Q1",
    },
];

export const teamData = [
    {
        img: "/images/landing/team/victor.png",
        name: "Victor Kaycee",
        role: "Co-founder / CEO",
        linkedin: "https://www.linkedin.com/in/victor-kaycee",
    },
    {
        img: "/images/landing/team/glory.png",
        name: "Glory Lucas",
        role: "Co-founder / CMO",
        linkedin: "https://linkedin.com/in/GloryLucas",
    },

    {
        img: "/images/landing/team/caleb.png",
        name: "Caleb Lucas",
        role: "Engineering Manager",
        linkedin: "https://www.linkedin.com/in/cmdlucas",
    },
    // {
    //   img: "/images/landing/team/ken.png",
    //   name: "Ken Suzuki",
    //   role: "Core Contributor",
    //
    // },
    {
        img: "/images/landing/team/jeffery.png",
        name: "Jeffery Pan",
        role: "Institutional Advisor from SoftBank",
        linkedin: "https://www.linkedin.com/in/nasa-b64b53215/",
    },
    {
        img: "/images/landing/team/pekka.png",
        name: "Pekka Kelkka",
        role: "Institutional Advisor",
        linkedin: "https://www.linkedin.com/in/pekkakelkka/",
    },
    {
        img: "/images/landing/team/sam.png",
        name: "Sam Vukmirica",
        role: "Gaming Advisor",
        linkedin: "https://www.linkedin.com/in/simo-vukmirica-001/",
    },
    {
        img: "/images/landing/team/robbie.png",
        name: "Robbie Joe",
        role: "Polygon Advisor / CEO Kommunitas",
        linkedin: "https://www.linkedin.com/in/robby678/",
    },
    {
        img: "/images/landing/team/anita.png",
        name: "Anita Kalergis",
        role: "Institutional Advisor",
        linkedin: "https://www.linkedin.com/in/anitakalergis/",
    },
];
